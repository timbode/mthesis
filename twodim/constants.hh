...
const double L=a_X/(N_[0] - 1); // for approx of membrane (droplet case) --- PROBLEM with loss of energy seems to be here!!!
// ------------------------------------------------------------------------------------------------

// ------------------------------------------------------------------------------------------------
// constants from particle.hh and droplet.hh
const double M=1.0;//N_[0]*N_[0]-4*N_[0]+4; // particle mass
const double D=0.4; // particle diameter
const double f=1e0; // "bouncing" frequency of particle, so 1/10 for every 10 seconds
// ------------------------------------------------------------------------------------------------
